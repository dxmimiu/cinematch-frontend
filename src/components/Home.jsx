...
export default function Home({ setStep, currentUser, userPreferences }) {
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        ...
        const [moviesRes, tvsRes, recRes] = await Promise.all([
          fetch(`https://api.themoviedb.org/3/trending/movie/week?api_key=${API_KEY}&language=th-TH`),
          fetch(`https://api.themoviedb.org/3/trending/tv/week?api_key=${API_KEY}&language=th-TH`),
          fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=th-TH&sort_by=vote_average.desc&vote_count.gte=1000&page=1`)
        ]);
        ...
        setTrendingMovies(moviesData.results.filter(item => item.poster_path).slice(0, 10));
        setTrendingTVs(tvsData.results.filter(item => item.poster_path).slice(0, 10));
        setRecommended(recData.results.filter(item => item.poster_path).slice(0, 10));
      } catch (error) {
  ...
  // ฟังก์ชันกด Like / Dislike แบบติดตั้งระบบรักษาความปลอดภัย JWT Token
const handleVote = async (movie, type) => {
  ...
  const moodTags = [
    { label: 'เดือดสุดระทึก', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z" /></svg> },
    { label: 'ตลกขบขันเบาสมอง', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    { label: 'โรแมนติกอบอุ่นหัวใจ', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg> },
    { label: 'สยองขวัญสั่นประสาท', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg> },
    { label: 'หักมุมเดาทางยาก', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg> }
  ];
      ...
      <div className="min-w-36 max-w-36 md:min-w-44 md:max-w-44 flex flex-col h-full group bg-white rounded-2xl p-2.5 shadow-[0_4px_15px_rgba(33,1,0,0.03)] border border-[#FECE79]/30 hover:shadow-md transition-shadow relative">
        <div onClick={() => handleMovieClick({...item, media_type: isTV ? 'tv' : 'movie'})} className="relative w-full aspect-3/4 rounded-xl overflow-hidden mb-3 cursor-pointer bg-[#FFFDF9] shrink-0">
          ...
          <div className="absolute top-2 right-2 bg-[#210100]/80 backdrop-blur-sm text-[#FECE79] text-xs font-black px-2 py-1 rounded-md flex items-center gap-1 z-10">
            <svg className="w-3 h-3 text-[#E6A341]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
        ...
        <div className="flex flex-col grow px-1">
          ...
          <p className="text-[#B14A36] font-bold text-[11px] md:text-xs mt-1.5 mb-3">{year || "N/A"} • {isTV ? 'TV Series' : 'Movie'}</p>
          <div className="mt-auto pt-2 border-t border-[#FECE79]/30 w-full">
            <button onClick={() => handleMovieClick({...item, media_type: isTV ? 'tv' : 'movie'})} className="w-full text-[#8C0902] font-extrabold text-xs text-center hover:text-[#B14A36] transition-colors py-1">
              ดูรายละเอียด
            </button>
          </div>
        </div>
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40">
        <div className="w-12 h-12 border-4 border-[#FECE79] border-t-[#8C0902] rounded-full animate-spin mb-5"></div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pb-20 animate-fade-in relative">
      <div className="max-w-7xl mx-auto px-6 mt-8 mb-16">
        <div className="bg-linear-to-br from-[#FECE79]/30 via-[#FFFDF9] to-[#E6A341]/10 rounded-3xl p-8 md:p-16 flex flex-col items-center justify-center border border-[#FECE79]/40 shadow-[0_10px_40px_rgba(230,163,65,0.08)] relative overflow-hidden text-center">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-[#210100] mb-8 tracking-tight leading-tight">
            ค้นหาภาพยนตร์ <br className="hidden sm:block"/>
            <span className="text-[#8C0902]">ที่ตรงใจคุณและเพื่อน</span>
          </h1>
          
          <div className="flex flex-col sm:flex-row items-center gap-4 justify-center">
            <button onClick={() => setStep(1)} className="bg-[#8C0902] hover:bg-[#210100] text-white font-extrabold text-sm md:text-base py-4 px-8 rounded-full transition-all shadow-[0_8px_20px_rgba(140,9,2,0.25)] hover:-translate-y-1 flex items-center gap-2 w-full sm:w-auto justify-center">
              <svg className="w-5 h-5 text-[#FECE79]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M15.362 5.214A8.252 8.252 0 0 1 12 21 8.25 8.25 0 0 1 6.038 7.047 8.287 8.287 0 0 0 9 9.601a8.983 8.983 0 0 1 3.361-6.866 8.21 8.21 0 0 0 3 2.48Z" /></svg>
              สร้างห้องจับคู่ (Duo)
            </button>
            <button onClick={() => setStep(4)} className="bg-white hover:bg-[#FFFDF9] border-2 border-[#FECE79] text-[#8C0902] font-extrabold text-sm md:text-base py-3.5 px-8 rounded-full transition-all hover:shadow-md w-full sm:w-auto justify-center">
              ไปหน้าค้นหาและฟิลเตอร์
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-16">
        <section>
          <p className="text-[#B14A36] font-bold text-sm mb-4">สำรวจตามมู้ดอารมณ์</p>
          <div className="flex flex-wrap gap-3">
            {moodTags.map((tag, i) => (
              <button key={i} onClick={() => setStep(4)} className="bg-white border border-[#FECE79]/60 hover:border-[#E6A341] text-[#210100] hover:text-[#8C0902] hover:bg-[#FECE79]/10 font-bold text-sm py-2 px-4 rounded-full transition-all hover:-translate-y-1 shadow-sm flex items-center gap-2">
                <span className="text-[#8C0902]">{tag.icon}</span>{tag.label}
              </button>
            ))}
          </div>
        </section>
        ...
        <section>
          <div className="flex items-end justify-between mb-6">
            <div>
              <h2 className="text-2xl font-black text-[#210100] tracking-tight">คัดสรรมาเพื่อคุณโดยเฉพาะ</h2>
              <p className="text-[#B14A36] font-medium text-sm mt-1">สวัสดีคุณ {currentUser?.name} อ้างอิงจากควิซความชอบของคุณ</p>
            </div>
            <ScrollButtons scrollRef={recScrollRef} />
          </div>
          <div ref={recScrollRef} className="flex overflow-x-auto gap-5 pb-6 pt-2 scrollbar-hide snap-x" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {recommended.map(item => (<div key={item.id} className="snap-start shrink-0"><MovieCard item={item} isTV={false} /></div>))}
          </div>
        </section>
        ...
        <section>
          <div className="flex items-end justify-between mb-6">
            <h2 className="text-2xl font-black text-[#210100] tracking-tight">ซีรีส์กำลังฮิต</h2>
